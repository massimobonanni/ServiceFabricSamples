...
namespace CartActor.Test
{
    ...
    {
        internal static CartActor CreateActor(ActorId id, IProductsService productsService = null)
        {
            if (productsService == null)
                productsService = new Mock<IProductsService>().Object;

            Func<ActorService, ActorId, ActorBase> actorFactory = (service, actorId) => new CartActor(service, id, productsService);
        ...
        public async Task AddProductAsync_ProductExists_ReturnOK()
        {
            ...
            var actor = CreateActor(id, productsService.Object);
            var stateManager = (MockActorStateManager)actor.StateManager;
